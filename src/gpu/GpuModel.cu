...
__global__ void kernel_conv_forward_gpu(float* output, const float* input, const float* weight, const int n_sample, const int channel_out, const int channel_in, const int height_in, const int width_in, const int height_kernel, const int stream_offset) 
{
    // Calculate indices
...
void GPU_Conv::conv_forward_gpu(float* output, const float* input, const float* weight, const int n_sample, const int channel_out, const int channel_in, const int height_in, const int width_in, const int height_kernel)
{
    ...
    for (int i = 0; i < n_streams; ++i) {
        ...
        kernel_conv_forward_gpu<<<num_blocks_in_grid, num_threads_per_block, channel_out*channel_in*height_kernel*height_kernel*sizeof(float), streams[i]>>>(device_output, device_input, device_weight, samples_per_stream, channel_out, channel_in, height_in, width_in, height_kernel, stream_offsets[i]);
