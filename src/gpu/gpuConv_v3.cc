#include "gpuConv_v3.h"
#include <math.h>
#include <iostream>
#include <memory>
#include "../gpu/GpuModel.h"

void gpuConv_v3::init() {
...
void gpuConv_v3::im2col(const Vector& image, Matrix& data_col) {
...
void gpuConv_v3::forward(const Matrix& bottom) {
    ...
    GPU_Conv gpu;
    printf("GPU 3 - Forward Convolution Start: \n");

    Timer timer;
    timer.Start();
    gpu.conv_forward_gpu_v3(output_data.get(), input_data.get(), weight_data.get(), n_sample, channel_out, channel_in, height_in, width_in, height_kernel);
    timer.Stop();
    printf("GPU 3 - Forward Convolution End - Time: %lf ms\n", timer.Elapsed());
}

// col2im, used for grad_bottom
// data_col size: Matrix (hw_out, hw_kernel * channel_in)
// image size: Vector (height_in * width_in * channel_in)
void gpuConv_v3::col2im(const Matrix& data_col, Vector& image) {
    int hw_in = height_in * width_in;
    int hw_kernel = height_kernel * width_kernel;
    int hw_out = height_out * width_out;
    // col2im
...
void gpuConv_v3::backward(const Matrix& bottom, const Matrix& grad_top) {
    int n_sample = bottom.cols();
    grad_weight.setZero();
    grad_bias.setZero();
...
void gpuConv_v3::update(Optimizer& opt) {
...
std::vector<float> gpuConv_v3::get_parameters() const {
...
void gpuConv_v3::set_parameters(const std::vector<float>& param) {
...
std::vector<float> gpuConv_v3::get_derivatives() const {
